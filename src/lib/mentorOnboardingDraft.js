const STORAGE_KEY = 'ai-cowork:mentor-onboarding:v1';
...
function writeStoredDraft(draft) {
  ...
  try {
    ...
    emitSyncState({
      error: 'Este navegador bloqueou o armazenamento local do formulário.',
      status: 'error',
    });
    return false;
  }
...
function toSyncError(error) {
  const message = error?.message || 'Não foi possível sincronizar o formulário.';
...
async function sendDraft(draft, submit) {
  ...
  if (!isOnline()) {
    ...
    throw new Error('Sem conexão com a internet. O formulário continua salvo neste dispositivo.');
  }

  emitSyncState({ error: null, status: submit ? 'submitting' : 'syncing' });

  const { data, error } = await supabase.rpc('save_mentor_onboarding', {
...
function enqueueSync(draft, submit = false) {
  ...
  syncChain = task.catch((error) => {
    if (isOnline()) emitSyncState({ error: error.message, status: 'error' });
  });
...
export function getOrCreateMentorOnboardingDraft() {
...
export function persistMentorOnboardingDraft(draft) {
  writeStoredDraft(draft);
  emitSyncState({
    error: null,
...
export function flushMentorOnboardingDraft(draft = readStoredDraft()) {
  if (!draft) return Promise.resolve();
  window.clearTimeout(debounceTimer);
  return enqueueSync(draft);
}

export async function submitMentorOnboarding(draft) {
  writeStoredDraft(draft);
  window.clearTimeout(debounceTimer);
...
export function subscribeToMentorOnboardingSync(listener) {
  listeners.add(listener);
  listener(syncState);
  return () => listeners.delete(listener);
}

export function startMentorOnboardingSync() {
  if (!isBrowser()) return () => {};
  ...
  const handleOnline = () => {
    ...
    if (draft) void flushMentorOnboardingDraft(draft).catch(() => undefined);
  };
